...
for epoch in range(config["epochs"]):
    ...
    for i, (x_batch, voxel_batch, obj_id_batch) in enumerate(train_loader):
        ...
        #fixed noise and timestep for overfit 
        noise = torch.ones_like(x_batch) * 0.1
        timesteps = torch.zeros(x_batch.shape[0], dtype=torch.long, device=device)
        ...
        print("Shapes:", noisy.shape, noise.shape, noise_pred.shape)
