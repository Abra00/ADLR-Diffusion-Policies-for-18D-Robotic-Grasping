...
# Settings
# -------------------------------
mesh_folder = "/Users/lucafrontini/Library/Mobile Documents/com~apple~CloudDocs/Uni/TUM/2. Semester /Advanced Deep Learning for Robotics/ADLR-Diffusion-Policies-for-18D-Robotic-Grasping/Data/studentGrasping/student_grasps_v1"  # Root folder containing the dataset
output_folder = "/Users/lucafrontini/Library/Mobile Documents/com~apple~CloudDocs/Uni/TUM/2. Semester /Advanced Deep Learning for Robotics/ADLR-Diffusion-Policies-for-18D-Robotic-Grasping/Data/studentGrasping/student_grasps_v1_processed"        # Folder to save SDFs and fixed meshes
os.makedirs(output_folder, exist_ok=True)

mesh_scale = 1       # Target range [-1, 1]
size = 32            # SDF grid resolution
level = 2 / size     # Step size for mesh2sdf
max_objects = 10     # For testing: limit number of instances to process (None for all)
...
for filepath in mesh_paths:
    ...
    bbox_size = (mesh.vertices.max(0) - mesh.vertices.min(0)).max()
...
for filepath in mesh_paths:
    ...
    # Scale relative to largest object
    vertices_scaled = (vertices - center) * scale_factor

    t0 = time.time()
    sdf, mesh_fixed = mesh2sdf.compute(
        vertices_scaled, mesh.faces, size=size, fix=True, level=level, return_mesh=True
    )

    sdf_min = sdf.min()
    sdf_max = sdf.max()
    print(f"{fname} | SDF: min={sdf_min:.6f}, max={sdf_max:.6f}")
    t1 = time.time()
