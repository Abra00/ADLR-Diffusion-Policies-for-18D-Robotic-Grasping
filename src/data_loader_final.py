...
def load_and_process_data(data_dir, batch_size, test_size=0.1):
    ...
    for f in files:
        try: 
            data = np.load(str(f))
            if len(data['voxel_sdf'].shape) == 3: voxel = voxel.reshape(1, *voxel.shape)
            all_objects.append({
                'voxel_grid': data['voxel_sdf'].astype(np.float32),
                'grasps' : data['grasps'],
                'object_id' : f.stem,
                'center' : data['center'],
                'scale' : data['scale']
            ...
            print(f'Problem during loading of all objects')
            continue
